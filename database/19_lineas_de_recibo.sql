...
begin
  ...
  values (
    ...
    0, 0, 0, coalesce(p_status, 'completed'),
    p_mileage, v_fecha
  )
...
grant execute on function public.dtek_admin_cerrar_trabajo(uuid, text, text, text, int, jsonb, boolean, text) to authenticated;
...
-- ---------------------------------------------------------------------
-- 7. El cliente ve el detalle de líneas en su historial (Garage)
--
--    dtek_client_list_my_work_orders ya existía (06_client_portal.sql) y
--    solo devolvía los tres totales. Se agrega el km del día y las líneas,
--    para que el recibo del cliente muestre lo mismo que armó el mecánico.
